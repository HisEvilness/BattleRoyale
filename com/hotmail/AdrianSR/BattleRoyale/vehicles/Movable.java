...
public class Movable {
	 ...
	 * Parts data.
	 */
	private final List < UUIDEntity < ? > > parts;
	 ...
	 */
	public Movable(final Location spawn, final boolean autoupdate, final boolean auto_model, final Plugin plugin) {
	...
	public Movable(final Location spawn, final List<Entity> parts, final boolean autoupdate, final boolean auto_model,
		...
		// spawn main stand.
		this.main = new UUIDArmorStand(world.spawn(spawn, ArmorStand.class));
		...
		if (autoupdate) {
			auto_updater = SchedulerUtil.runTaskTimer ( ( ) -> {
				// check main stand.
...
//				final ArmorStand stand = getMainEntity().get();
	...
	protected void model() {
		// make main stand.
		main = new UUIDArmorStand(world.spawn(spawn, ArmorStand.class));
		...
		for (int x = 0; x < 1; x++) {
			...
			// make part.
			final UUIDArmorStand part = new UUIDArmorStand(world.spawn(to, ArmorStand.class));
	 ...
	 */
	public UUIDEntity < ArmorStand > addPart(final Location spawn, final boolean visible, boolean gravity) {
	...
	public < T extends Entity > UUIDEntity < T > addPart(final Class<T> clazz, final Location spawn, final boolean visible, boolean gravity) {
		...
		if (!gravity && !(ent instanceof ArmorStand)) {
			// when version is >= 1.10
			if (Version.getServerVersion().isNewerEquals(Version.v1_10_R1)) {
				// remove gravity.
				...
				if (ent instanceof LivingEntity) {
					...
					EntityReflection.setAI((LivingEntity) ent, false);
				}
			}
		}
		
		// add part.
		return addPart ( clazz , ent , visible );
	}
	 ...
	 * @return The added {@link UpdatableEntity} part.
	 */
	public < T extends Entity > UUIDEntity < T > addPart ( Class < T > entity_class , final Entity entity , final boolean visible ) {
		// make.
		final UUIDEntity < T > part = new UUIDEntity < > ( entity );
	...
	public void removePart(final Entity partEntity) {
		removePart ( new UUIDEntity < > ( partEntity ) );
	}
	 ...
	 */
	public void removePart ( final UUIDEntity < ? > part ) {
		final Iterator < UUIDEntity < ? > > iterator = parts.iterator ( );
		
		while ( iterator.hasNext ( ) ) {
			UUIDEntity < ? > other = iterator.next ( );
			if ( other.getUniqueId ( ).equals ( part.getUniqueId ( ) ) ) {
				iterator.remove ( );
			}
		}
	}
...
//				setLocation(LocationUtils.setYaw(to, (to.getYaw() + 10.0F)));
	...
	public void setLocation(final Location to) {
		...
		final List < UUIDEntity < ? > > targets = new ArrayList< UUIDEntity < ? > >(Arrays.asList(main));

		// add other targets.
		if (!customParts.isEmpty()) {
			for (Entity e : customParts) {
				if (e != null) {
					targets.add(new UUIDEntity<>(e));
				}
			}
		} else {
			for (UUIDEntity < ? > part : parts) {
				targets.add(part);
			}
		}

		// move.
		for (UUIDEntity < ? > target : targets) {
			// check is not null.
			if (target == null) {
				continue;
			}
			...
			if (entity instanceof ArmorStand) {
				...
				EntityReflection.setVisible(entity, !visible);
			}

			// move depending of stand.
			if (isMain) {
				// set location.
				EntityReflection.setLocation(entity, to.getX(), to.getY(), to.getZ(), to.getYaw(), 0.0F);
			} else {
				// get part spawn.
...
//				float lookYaw = DirectionUtils.pointLocationTo(mainSpawn, partSpawn);
				float lookYaw = DirectionUtil.lookAt ( mainSpawn , partSpawn ) [ 0 ];
				float grados  = (lookYaw < 0 ? (360.0F - Math.abs(lookYaw)) : lookYaw);
				float dife    = ((grados + addedYaw) - yaw);
				
				// get direction.
				Vector locationDirection = VectorUtil.rotateVector(from.getDirection(), dife, 0F);
				...
				// set location.
				EntityReflection.setLocation(entity, X, Y, Z, Yaw, 0.0F);
			}
			...
			if (entity instanceof ArmorStand) {
				EntityReflection.setInvulnerable ( ( (ArmorStand) entity ) , true );
			}

			// update location.
			if (isMain) {
				// get location.
				final Location currentLocation = EntityReflection.getLocation(entity);
...
//		}
	...
	public List < UUIDEntity < ? > > getParts ( ) {
		return parts;
	}
	...
	public UUIDArmorStand getMainEntity() {
		return main;
	}
